...
public class TaskListPanelHandle extends GuiHandle {
    ...
    public boolean containsInOrder(int startPosition, ReadOnlyTask... tasks) {
        ...
        for (int i = 0; i < tasks.length; i++) {
            if (!tasksInList.get(startPosition + i).getName().getValue().equals(tasks[i].getName().getValue())) {
                return false;
            }
        }
    ...
    public TaskCardHandle navigateToTask(String name) {
        ...
        final Optional<ReadOnlyTask> task = getListView().getItems().stream()
                .filter(p -> p.getName().getValue().equals(name)).findAny();
        if (!task.isPresent()) {
    ...
    public TaskCardHandle getTaskCardHandle(int index) throws IllegalValueException {
        return getTaskCardHandle(new TaskBuilder(getTask(index))
                .build());
    }
